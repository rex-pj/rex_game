...
<div class="quiz-container">
  ...
  {:else if $quizGameState === "completed"}
    ...
    <div class="results-container">
      <div class="results-card">
        <h2><i class="fa-solid fa-trophy"></i> Hoàn thành Màn {$quizStats.level}!</h2>
        ...
        <div class="results-stats">
          ...
          <div class="result-item">
            ...
            <span class="result-value correct">{$quizStats.correctAnswers}/{$quizStats.totalQuestions}</span>
          </div>
          <div class="result-item">
            <span class="result-icon"><i class="fa-solid fa-chart-simple"></i></span>
            <span class="result-label">Độ chính xác</span>
            <span class="result-value">{getAccuracy($quizStats.correctAnswers, $quizStats.totalQuestions)}%</span>
          </div>
          <div class="result-item">
            <span class="result-icon"><i class="fa-solid fa-stopwatch"></i></span>
            <span class="result-label">Thời gian</span>
            <span class="result-value">{formatTime($quizStats.timeElapsed)}</span>
          </div>
        </div>
    ...
    {#if $currentQuestion}
      <div class="question-area">
        ...
        {#if $quizGameState === "answered" && $currentQuestion.answered}
          <div class="feedback {$currentQuestion.isCorrect ? 'correct' : 'incorrect'}">
            ...
            {:else}
              ...
              <span>Sai rồi! Đáp án đúng: <strong>{$currentQuestion.correctAnswer}</strong></span>
            {/if}
          </div>
        {/if}
      </div>
  ...
  {#if $quizNewAchievements.length > 0}
    <div class="achievements-toast">
      ...
      {/each}
      <button class="btn-dismiss" onclick={() => clearQuizAchievements()} aria-label="Đóng"><i class="fa-solid fa-xmark"></i></button>
    </div>
  {/if}
</div>
